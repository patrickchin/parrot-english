...
| --- | --- | --- |
| Build a one-page interactive speaking experience. | Keep the first screen as a full-screen lesson stage with character-led scenes, microphone recording, evaluation, and audible feedback. | `src/app/App.tsx`, `src/lessons/LessonPlayerUi.tsx` |
...
| Make lessons easy to add and remove. | Store each lesson in `content/lessons/*.json` and discover files automatically for the picker. | `src/lessons/lesson-catalog.ts`, `content/lessons` |
...
| Let families create their own lessons. | Generate or paste editable lesson JSON into one main form, normalize recoverable issues into warnings, store it per authenticated user in D1, and reuse the existing player. | `src/lessons/LessonCreator.tsx`, `worker/my-lessons.ts`, `learner_lesson` |
...
| Keep realtime profile persistence prose-only. | Finalize the agent's saved paragraph automatically when a conversation ends. Complete onboarding only when name and age readiness signals are present; otherwise keep the session-scoped bypass. Reject structured candidates while retaining the deployed state-ingest route and dormant table for rollback safety. | `src/conversation/usePeppaConversation.ts`, `worker/conversations.ts`, `worker/conversation-repository.ts` |
| Make global controls consistent. | Keep the authenticated account bar global in `AuthGate` and render every top-level navigation control with shared React header primitives. Route content must not redefine header geometry or typography. | `src/auth/AuthGate.tsx`, `src/app/AppHeader.tsx` |
| Use one frontend styling architecture. | Use Tailwind 4 utilities directly in components, centralize repeated controls in small React primitives, and keep the CSS entrypoint to Tailwind configuration and browser-level behavior. Do not hide long utility strings in TypeScript constants. | `src/shared/ui.tsx`, `src/app/AppHeader.tsx`, `src/lessons/LessonPlayerUi.tsx`, `src/styles.css` |
| Share lesson-player presentation. | Keep the HUD, character presentation, speech, start action, control dock, and errors in domain React components. Retain custom CSS only for runtime slot calculation, the speech-tail polygon, and one combined width/height placement rule. | `src/lessons/LessonPlayerUi.tsx`, `src/lesson.css` |
...
| Treat mobile headers as one row. | At supported narrow and short viewports, Profile, Log out, and page navigation must stay visible, aligned, non-overlapping, and horizontally contained; account actions remain fixed while page content scrolls. | `tests/e2e/header.spec.ts`, `src/app/AppHeader.tsx` |
...
- Lesson-player presentation is shared through `LessonPlayerUi`; `lesson.css`
  contains no general control, typography, color, or spacing system.
...
- The lesson player retains a tiny stylesheet for two browser features that do
  not map cleanly to ordinary utilities: runtime-count slot positioning and the
  polygon speech tail. Its short-wide override preserves the coupled viewport
  behavior covered by Playwright.
