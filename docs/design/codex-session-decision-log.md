...
| Let families create their own lessons. | Generate or upload editable lesson JSON, normalize recoverable issues into warnings, store it per authenticated user in D1, and reuse the existing player. | `src/lessons/LessonCreator.tsx`, `worker/my-lessons.ts`, `learner_lesson` |
