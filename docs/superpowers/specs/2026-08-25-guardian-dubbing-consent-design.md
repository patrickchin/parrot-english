...
available. The 24-line activity keeps its automatic first-example playback,
**Replay example** control, live waveform, **Hear my voice** review, retry,
retake, and final playback. The final screen may offer **Record another take**
and a saved-line picker because replacing one's performance is part of the
activity; it must not call that area grown-up options or expose deletion.
...
The obsolete v1 API family is not a guardian mutation surface. Its storage
prefix remains private and uncounted, and only a current-v2 guardian revoke or
account deletion retires it.
...
Audio reads require a current grant and accept current-v2-prefix objects only
when their grant generation matches. Because durable v2 has never shipped,
the first durable grant may adopt eligible clips already written under the
current `five-little-ducks-v2/` prefix. Clips under the legacy
`five-little-ducks-v1/` prefix are never adopted, listed, counted, or played.
A successful current-v2 revocation tombstones all 24 current slots and retires
the legacy marker plus its nine slots before a later grant is possible. This
prevents old or partially revoked clips from crossing consent generations while
preserving the one-time current-prefix migration path.

Once the durable account-deletion tombstone exists, consent grant, status, and
audio all fail closed. Status and audio check before any R2 work and again
before responding; grant checks before and after its D1 mutation. Upload and
reset keep their existing deletion fencing. Account deletion retains the
current marker and 24 slot fences plus the legacy marker and nine slot fences
(35 closure objects total). All owner scoping, MIME/signature validation, body
limits, CAS generation handling, and private `no-store` headers remain in
force.
...
- account-deletion-pending grant/status/audio requests fail closed, including a
  tombstone race before response.
- legacy v1-prefix clips remain invisible after the first durable v2 grant.
