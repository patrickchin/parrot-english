...
zoom 1. The browser may magnify the complete 360 × 240 canvas at an integer 1×
on mobile, 2× on normal desktops, or 3× on wide screens, so every asset is
magnified together and the game fills most of the available window. Idle is
one stable frame, while movement and emotes use centered, foot-aligned cycles.
