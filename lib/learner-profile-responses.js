...
/** @param {Record<string, unknown>} value @param {Set<string>} expected */
...
/** @param {{ answersJson: string }} profile */
export function readV2Answers(profile) {
  ...
  if (
    ...
    envelope.questionnaireVersion !== QUESTIONNAIRE_VERSION ||
    !isRecord(responses) ||
    !Object.values(responses).every(isResponse) ||
    !(envelope.description === null || isTrimmedText(envelope.description, 2_000))
  ) {
    throw new Error(PROFILE_ERROR);
  }
...
/** @param {{ skippedQuestionKeysJson: string }} profile */
...
/**
 * @template {{ skippedQuestionKeysJson: string }} Profile
...
export function skipProfileQuestion(profile, answerKey) {
  return /** @type {Profile} */ ({
    ...profile,
    skippedQuestionKeysJson: JSON.stringify([
      ...new Set([...readSkipped(profile), answerKey]),
    ]),
  });
}
 ...
 * @param {{ answersJson: string, skippedQuestionKeysJson: string }} profile
