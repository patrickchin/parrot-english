...
export const CUSTOM_LESSON_BACKGROUND_IDS = [
  "episode-garden",
  "meadow-day",
  "meadow-evening",
  "reward",
...
const REQUIRED_STEP_KEYS = ["dialogue", "speaker"];
...
const REQUIRED_RESPONSE_KEYS = ["after", "dialogue", "speaker"];
 ...
 * }} VisualCatalog
 * @typedef {"correct" | "incorrect" | "noInput"} SpeechCheckOutcome
 * @typedef {"retry" | "continue"} ResponseAction
 * @typedef {{ speaker: "peppa" | "dolly" | "narrator", dialogue: string, emotes?: Record<string, string>, after: ResponseAction }} LessonResponse
 * @typedef {{ maxAttempts: number, correct: LessonResponse, incorrect: LessonResponse, incorrectFinal: LessonResponse, noInput?: LessonResponse, noInputFinal?: LessonResponse }} LessonCheck
 * @typedef {{ speaker: "peppa" | "dolly" | "user" | "narrator", dialogue: string, emotes?: Record<string, string>, check?: LessonCheck }} LessonStep
 ...
 * @param {string} path
 */
function validatePartialEmotes(value, characterIds, catalog, path) {
  if (value === undefined) return;
  requireRecord(value, path);

  for (const characterId of characterIds.filter((id) => id !== "user")) {
    const emote = value[characterId];
    if (emote === undefined) continue;
    requireText(emote, `${path}.${characterId}`);
    if (!catalog.emotes.has(emote)) {
      throw new Error(`${path}.${characterId} is not supported`);
    }
    if (!catalog.characters.get(characterId)?.assets[emote]) {
      throw new Error(`${path}.${characterId} has no visual asset`);
    }
  }
}
 ...
 * @param {VisualCatalog} catalog
 * @param {string} path
 * @param {{ final?: boolean, correct?: boolean }} [options]
 * @returns {LessonResponse}
 */
function validateResponse(value, characterIds, catalog, path, options = {}) {
  requireRecord(value, path);
  requireKeys(value, REQUIRED_RESPONSE_KEYS, path);
  requireText(value.speaker, `${path}.speaker`);
  if (
    value.speaker === "user" ||
    (value.speaker !== "narrator" && !catalog.characters.has(value.speaker))
  ) {
    throw new Error(`${path}.speaker must be narrator or a supported non-user character`);
  }
  requireText(value.dialogue, `${path}.dialogue`);
  validatePartialEmotes(value.emotes, characterIds, catalog, `${path}.emotes`);
  requireText(value.after, `${path}.after`);
  if (value.after !== "retry" && value.after !== "continue") {
    throw new Error(`${path}.after must be retry or continue`);
  }
  if ((options.final || options.correct) && value.after !== "continue") {
    throw new Error(`${path}.after must be continue`);
  }

  return /** @type {LessonResponse} */ (value);
}
 ...
 * @returns {LessonCheck}
 */
function validateCheck(value, characterIds, catalog, path) {
  requireRecord(value, path);
  requireKeys(value, REQUIRED_CHECK_KEYS, path);
  if (
    !Number.isInteger(value.maxAttempts) ||
    Number(value.maxAttempts) < 1 ||
    Number(value.maxAttempts) > 5
  ) {
    throw new Error(`${path}.maxAttempts must be an integer between 1 and 5`);
  }

  validateResponse(value.correct, characterIds, catalog, `${path}.correct`, {
    correct: true,
  });
  validateResponse(value.incorrect, characterIds, catalog, `${path}.incorrect`);
  validateResponse(
    value.incorrectFinal,
    characterIds,
    catalog,
    `${path}.incorrectFinal`,
    { final: true },
  );
  if (value.noInput !== undefined) {
    validateResponse(value.noInput, characterIds, catalog, `${path}.noInput`);
  }
  if (value.noInputFinal !== undefined) {
    validateResponse(
      value.noInputFinal,
      characterIds,
      catalog,
...
export function createLessonCatalog({ emotes, characters, backgrounds }) {
  ...
  const uniqueEmoteIds = new Set(emoteIds);
  if (uniqueEmoteIds.size !== emoteIds.length) {
    throw new Error("emotes must contain unique values");
  }
  const missingUniversalEmotes = LESSON_EMOTES.filter(
    (emote) => !uniqueEmoteIds.has(emote),
  );
  if (missingUniversalEmotes.length > 0) {
    throw new Error(
      `emotes must include: ${missingUniversalEmotes.join(", ")}`,
    );
  }
  ...
  characters.forEach((value, index) => {
    ...
    requireKeys(value.assets, LESSON_EMOTES, `${path}.assets`);

    /** @type {Record<string, VisualAsset>} */
    const assets = {};
    for (const emote of Object.keys(value.assets)) {
      if (!uniqueEmoteIds.has(emote)) {
        throw new Error(`${path}.assets.${emote} is not in the emote catalog`);
      }
...
/**
 * @param {{ characters: unknown, backgrounds: unknown }} input
 * @returns {VisualCatalog}
 */
export function createCustomLessonCatalog({ characters, backgrounds }) {
  const universalEmotes = new Set(LESSON_EMOTES);
  const reusableBackgrounds = new Set(CUSTOM_LESSON_BACKGROUND_IDS);
  const customCharacters = Array.isArray(characters)
    ? characters.map((character) => {
        if (!isRecord(character) || !isRecord(character.assets)) {
          return character;
        }
        return {
          ...character,
          assets: Object.fromEntries(
            Object.entries(character.assets).filter(([emote]) =>
              universalEmotes.has(emote),
            ),
          ),
        };
      })
    : characters;
  const customBackgrounds = Array.isArray(backgrounds)
    ? backgrounds.filter(
        (background) =>
          !isRecord(background) ||
          typeof background.id !== "string" ||
          reusableBackgrounds.has(background.id),
      )
    : backgrounds;

  return createLessonCatalog({
    emotes: LESSON_EMOTES,
...
export function validateLesson(value, catalog, sourceName) {
  ...
  value.scenes.forEach((sceneValue, sceneIndex) => {
    ...
    steps.forEach((stepValue, stepIndex) => {
      ...
      requireKeys(stepValue, REQUIRED_STEP_KEYS, stepPath);
      ...
      if (stepValue.check !== undefined) {
        if (stepValue.speaker !== "user") {
          throw new Error(`${stepPath}.check is only supported on a user step`);
        }
        validateCheck(stepValue.check, characterIds, catalog, `${stepPath}.check`);
      }
    });
  });
    ...
    stepValues.forEach((stepCandidate, stepIndex) => {
      ...
      if (hasEmoteObject) {
        for (const characterId of characters) {
          ...
          if (emoteCandidate === undefined) continue;
          const emote =
      ...
      let check;
      if (stepCandidate.check !== undefined) {
        if (speaker !== "user") {
          warn(`${stepPath}.check`, "ignored check on a non-user step");
        } else {
          try {
            check = validateCheck(
              stepCandidate.check,
            ...
            const message = error instanceof Error ? error.message : "invalid check";
            warn(`${stepPath}.check`, `ignored invalid check: ${message}`);
          }
        }
      }

      const normalizedStep = /** @type {LessonStep} */ ({
        ...stepCandidate,
        speaker,
        dialogue: stepCandidate.dialogue,
        emotes,
      });
      if (check) normalizedStep.check = check;
      else delete normalizedStep.check;
