...
const REQUIRED_CHARACTERS = ["peppa", "dolly", "user"];
...
export function validateLesson(value, catalog, sourceName) {
  ...
  value.scenes.forEach((sceneValue, sceneIndex) => {
    ...
    steps.forEach((stepValue, stepIndex) => {
      ...
      if (stepValue.speaker !== "narrator" && !characterIds.includes(stepValue.speaker)) {
        throw new Error(`${stepPath}.speaker must be narrator or a visible character`);
      }
