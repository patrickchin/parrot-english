...
const RESPONSE_ERROR = "Invalid onboarding response.";
...
export function readV2Answers(profile) {
  ...
  if (
    ...
    !isRecord(envelope.responses) ||
    !Object.values(envelope.responses).every(isResponse) ||
    !(
...
/**
 * @template {Record<string, any>} Profile
...
export function ensureV2Profile(
  ...
  if (profile.onboardingStatus === "completed" && !forProfileEdit) {
    return profile;
  }
  ...
  const restarting = profile.onboardingStatus !== "completed";
  return /** @type {Profile} */ ({
    ...profile,
    ...
    onboardingStatus: restarting ? "not_started" : profile.onboardingStatus,
...
export function writeV2Response(profile, question, enrichment) {
  ...
  if (question.canonicalField === "age") {
    ...
    if (!Number.isInteger(age) || /** @type {number} */ (age) < 3 || age > 17) {
      throw new Error(RESPONSE_ERROR);
    }
    updated.age = age;
  }
