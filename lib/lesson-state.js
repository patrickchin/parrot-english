 ...
 * @property {"speaking" | "responding" | null} resumePhase
 * @typedef {
 *   | { type: "PLAY_SCENE" }
 *   | { type: "PAUSE_SCENE" }
 *   | { type: "SCENE_PREVIOUS" }
...
export function reduceLessonState(state, event, lesson) {
  switch (event.type) {
    case "PLAY_SCENE": {
      if (state.phase === LessonPhase.Paused && state.resumePhase) {
        return {
          ...state,
    ...
    case "PAUSE_SCENE": {
      ...
      return {
        ...
        resumePhase:
          state.phase === LessonPhase.Responding ? "responding" : "speaking",
      };
    }
    case "SCENE_PREVIOUS":
    ...
    case "SKIP_USER":
      return state.phase === LessonPhase.WaitingForUser ||
        state.phase === LessonPhase.Recording
        ? advanceScriptPosition(state, lesson, true)
        : state;
