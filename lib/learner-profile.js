...
export function readSkippedQuestionKeys(profile) {
  let skippedQuestionKeys;
  try {
    skippedQuestionKeys = JSON.parse(profile.skippedQuestionKeysJson ?? "[]");
  } catch {
    throw new Error("Invalid learner profile data.");
  }
  if (
    !Array.isArray(skippedQuestionKeys) ||
    skippedQuestionKeys.some((entry) => typeof entry !== "string")
  ) {
    throw new Error("Invalid learner profile data.");
  }
  return [...new Set(skippedQuestionKeys)];
}

/**
 * @template {{ skippedQuestionKeysJson?: string | null }} Profile
 * @param {Profile} profile
 * @param {string} answerKey
 * @returns {Profile}
 */
export function skipProfileQuestion(profile, answerKey) {
  const skippedQuestionKeys = new Set(readSkippedQuestionKeys(profile));
  skippedQuestionKeys.add(answerKey);
  return /** @type {Profile} */ ({
    ...profile,
    skippedQuestionKeysJson: JSON.stringify([...skippedQuestionKeys]),
  });
}
 ...
 * @returns {Profile}
 */
function unskipProfileQuestion(profile, answerKey) {
  const skippedQuestionKeys = readSkippedQuestionKeys(profile).filter(
    (entry) => entry !== answerKey,
  );
  if (!("skippedQuestionKeysJson" in profile)) return profile;
  return /** @type {Profile} */ ({
    ...profile,
    skippedQuestionKeysJson: JSON.stringify(skippedQuestionKeys),
  });
...
/**
 * @template {{ answersJson?: string | null, name?: string | null, age?: number | null, skippedQuestionKeysJson?: string | null }} Profile
 * @param {Profile} profile
 * @param {string} answerKey
 * @param {unknown} value
 * @returns {Profile}
...
export function writeProfileAnswer(profile, answerKey, value) {
  const unskippedProfile = unskipProfileQuestion(profile, answerKey);
  const answers = readProfileAnswers(unskippedProfile);
  delete answers.name;
  delete answers.age;
 ...
 * @param {{ answers: Record<string, unknown>, currentQuestionKey?: string | null, questions: Question[], skippedQuestionKeys?: string[] }} input
 */
export function getNextQuestion({
  ...
  const skipped = new Set(skippedQuestionKeys);
  /** @param {Question} entry */
  const isHandled = (entry) =>
    hasAnswer(answers[entry.answerKey]) ||
    (!entry.required && skipped.has(entry.answerKey));
  const saved = applicable.find(
    (entry) =>
      entry.answerKey === currentQuestionKey && !isHandled(entry),
  );
  if (saved) return saved;

  return applicable.find((entry) => !isHandled(entry)) ?? null;
}

/**
 * @param {{ answerKey: string, position?: number, branchingJson?: string | null, required?: boolean | number }[]} questions
 * @param {Record<string, unknown>} answers
 * @param {string | null | undefined} currentQuestionKey
 * @param {string[]} [skippedQuestionKeys]
 */
...
/**
 * @param {{ profileStatus: string, questionnaireVersion?: number | null }} profile
...
export function assignQuestionnaireVersion(profile, activeVersion) {
  ...
  if (profile.profileStatus === "completed") return profile.questionnaireVersion ?? null;
  return activeVersion;
}

/**
 * @param {{ profileStatus: string, lastSkippedSessionId?: string | null }} profile
