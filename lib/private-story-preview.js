...
const PRIVATE_STORY_SOURCE_BYTE_LIMIT = 131_072;
...
async function readStableFile(filePath, expectedStats) {
  let fileHandle;
  try {
...
async function readPrivateText({ filePath, stats }) {
  if (!stats) throw new Error("Unable to read private story text");
  try {
    requirePrivateStorySourceByteLimit(stats.size);
    return decodePrivateStorySource(await readStableFile(filePath, stats));
  } catch (error) {
    if (
...
export async function loadPrivateStoryPreview({
  ...
  try {
    ...
    manifest = JSON.parse(
      (await readStableFile(manifestPath, inspection.stats)).toString("utf8"),
    );
  } catch (error) {
    if (error?.message === PRIVATE_PATH_ERROR) throw error;
  ...
  for (const [index, value] of manifest.stories.entries()) {
    ...
    const paginated = paginatePrivateStoryText(await readPrivateText(textFile));
  ...
  for (const { id, paginated, title } of validatedStories) {
    ...
    excerptSourceUnits.push(...sourceUnits(paginated.body).units);
    for (const [pageIndex, text] of paginated.pages.entries()) {
      ...
      const source = audioInspection.exists
        ? await readStableFile(outputFilePath, audioInspection.stats)
        : null;
