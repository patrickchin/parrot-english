...
function privateTextFileIdentity(stats) {
  ...
  if (
    ...
    throw new Error(
      "Unable to verify that private story text files are distinct",
    );
  }
...
export async function loadPrivateStoryPreview({
  ...
  const textFileIdentities = new Set();
  ...
  for (const [index, value] of manifest.stories.entries()) {
    ...
    const textFileIdentity = privateTextFileIdentity(textFile.stats);
    if (textFileIdentity && textFileIdentities.has(textFileIdentity)) {
      throw new Error("Private story preview manifest has duplicate text file");
    }
    if (textFileIdentity) textFileIdentities.add(textFileIdentity);
