...
export function paginatePrivateStoryText(rawText) {
  ...
  for (const [unitIndex, unit] of units.entries()) {
    ...
    if (page.length && pageWords + unitWords > PAGE_WORD_LIMIT) {
      ...
      pageDelimiters.push(delimiters[unitIndex - 1]);
      page = [];
      pageWords = 0;
    }
  ...
  const rebuiltBody = pages.reduce(
    (result, pageText, pageIndex) =>
      result + (pageIndex ? pageDelimiters[pageIndex - 1] : "") + pageText,
    "",
  );
  if (normalizeStoryBody(rebuiltBody) !== normalizeStoryBody(sourceBody)) {
    throw new Error("Private story pagination changed normalized content");
  }
  return { body, pageSeparators: pageDelimiters, pages, title };
}
...
async function resolvePreviewFile(previewDirectory, filename) {
  if (
  ...
  try {
    const [directoryPath, filePath, fileStats] = await Promise.all([
...
export async function loadPrivateStoryPreview({
  ...
  for (const [index, value] of manifest.stories.entries()) {
    ...
    const textFilePath = await resolvePreviewFile(directory, story.textFile);
