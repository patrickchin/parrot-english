...
const DUB_GUIDE_AUDIO_LINES = Object.fromEntries([
  ...
  [3, "Mother duck said, “Quack, quack, quack, quack.”"],
  [4, "But only four little ducks came back."],
  [5, "Four little ducks went out one day."],
  [8, "But only three little ducks came back."],
  [9, "Three little ducks went out one day."],
  [12, "But only two little ducks came back."],
  [13, "Two little ducks went out one day."],
  [16, "But only one little duck came back."],
  [17, "One little duck went out one day."],
  [20, "But none of the five little ducks came back."],
  [21, "Sad mother duck went out one day."],
  [23, "Sad mother duck said, “Quack, quack, quack, quack.”"],
  [24, "And all of the five little ducks came back."],
].map(([lineNumber, text]) => {
  const id = "five-little-ducks-v2-guide-line-" + lineNumber;
  return [id, {
    speaker: "narrator",
    lang: "en-US",
    src: STATIC_AUDIO_BASE_PATH + "/" + id + ".mp3",
...
const oldMacDonaldGuideTexts = new Set();
const OLD_MACDONALD_GUIDE_AUDIO_LINES = Object.fromEntries(
  OLD_MACDONALD_DUB.lines.flatMap(({ text }, index) => {
    if (oldMacDonaldGuideTexts.has(text)) return [];
    oldMacDonaldGuideTexts.add(text);
    const id = `old-macdonald-v1-guide-line-${index + 1}`;
    return [[id, {
      speaker: "narrator",
      lang: "en-US",
      src: `${STATIC_AUDIO_BASE_PATH}/${id}.mp3`,
...
const CATALOG_DUB_GUIDE_AUDIO_LINES = Object.fromEntries(
  DUB_DEFINITIONS.flatMap((definition) => {
    const seen = new Set();
    return definition.lines.flatMap(({ text }, index) => {
      if (seen.has(text)) return [];
      seen.add(text);
      const id = `${definition.guideAudioPrefix}line-${index + 1}`;
      return [[id, {
        speaker: "narrator",
        lang: "en-US",
        src: `${STATIC_AUDIO_BASE_PATH}/${id}.mp3`,
...
export const STATIC_AUDIO_LINES = {
  ...
  ...OLD_MACDONALD_GUIDE_AUDIO_LINES,
  ...CATALOG_DUB_GUIDE_AUDIO_LINES,
  ...LESSON_AUDIO_LINES,
  ...STORY_AUDIO_LINES,
};
