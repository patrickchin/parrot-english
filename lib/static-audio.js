...
const CATALOG_DUB_GUIDE_AUDIO_LINES = Object.fromEntries(
  DUB_DEFINITIONS.flatMap((definition) => {
    ...
    return definition.lines.flatMap(({ text }, index) => {
      ...
      seen.add(text);
      const id = `${definition.id}-guide-line-${index + 1}`;
      return [[id, {
        speaker: "narrator",
        lang: "en-US",
        src: `${STATIC_AUDIO_BASE_PATH}/${id}.mp3`,
...
function createWordGameAudioLine({ id, source, text }, direction) {
  const spokenText = id === "word-game-animals-cat-prompt"
    ? text.replace(/^Cat\b/, "Kat")
    : text;
  ...
  return [id, {
    ...
    src: source,
    text,
    ttsText: `[${direction}] ${spokenText}`,
    voiceStyle: "energetic-character",
  }];
}

const WORD_GAME_AUDIO_LINES = Object.fromEntries([
  ...WORD_GAME_TOPICS.flatMap(({ items }) => items.flatMap(({ audio }) => [
    createWordGameAudioLine(audio.prompt, "excited, playful delivery for a young child; say the target word first, then ask with curiosity"),
    createWordGameAudioLine(audio.label, "bright, playful teaching delivery for a young child"),
    createWordGameAudioLine(audio.correct, "joyful, enthusiastic encouragement for a young child"),
  ])),
  createWordGameAudioLine(WORD_GAME_RETRY_AUDIO, "gentle, upbeat encouragement for a young child"),
  createWordGameAudioLine(WORD_GAME_COMPLETE_AUDIO, "happy, excited, not-loud celebration for a young child"),
]);
