 ...
 * @param {string} direction
 */
function addWordGameAudioLine(line, direction) {
  const existing = wordGameAudioById.get(line.id);
  if (existing) {
    if (existing.text !== line.text) {
      throw new Error(`Conflicting word-game audio text for ID: ${line.id}`);
    }
    return;
  }
  wordGameAudioById.set(
    line.id,
    createWordGameAudioLine(line, direction)[1],
  );
}

for (const { items } of WORD_GAME_CATEGORIES) {
  for (const { audio } of items) {
    addWordGameAudioLine(
      audio,
...
const WORD_GAME_AUDIO_LINES = Object.fromEntries(wordGameAudioById);
