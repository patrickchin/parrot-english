...
const WORD_GAME_AUDIO_LINES = Object.fromEntries([
  ...WORD_GAME_TOPICS.flatMap(({ items }) => items.flatMap(({ audio }) => [
    createWordGameAudioLine(audio.prompt, "warm, playful, clear curiosity"),
    createWordGameAudioLine(audio.label, "clear first-word teaching"),
    createWordGameAudioLine(audio.correct, "bright encouragement"),
  ])),
  createWordGameAudioLine(WORD_GAME_RETRY_AUDIO, "gentle encouragement"),
  createWordGameAudioLine(WORD_GAME_COMPLETE_AUDIO, "happy, not-loud celebration"),
]);
