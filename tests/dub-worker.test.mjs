...
const OWNER_RECORDINGS_PREFIX = `${ACCOUNT_PREFIX}learners/Mary/recordings/`;
const OWNER_PREFIX = `${OWNER_RECORDINGS_PREFIX}nursery-rhymes/five-little-ducks-v2/`;
const MARKER_KEY = `${OWNER_PREFIX}.dub-generation`;
const slotKey = (lineId) => `${OWNER_PREFIX}${lineId}.audio`;
const OLD_OWNER_PREFIX = `${OWNER_RECORDINGS_PREFIX}nursery-rhymes/old-macdonald-v1/`;
...
const SIBLING_PREFIX = `${ACCOUNT_PREFIX}learners/Rose/recordings/nursery-rhymes/five-little-ducks-v2/`;
...
function storedAudio(item) {
  return item.bytes.subarray(
    Number(item.options.customMetadata.payloadOffset ?? 0),
  );
}
...
function assertResetTombstones(bucket, generation) {
  ...
  for (const lineId of LINE_IDS) {
    ...
    assert.deepEqual(
      item?.bytes,
      fenceBytes("slot", generation, "tombstone"),
      lineId,
    );
    assert.deepEqual(
  ...
  return {
    ...
    async put(key, bytes, options) {
      ...
      const item = {
        ...
        etag: contentEtag(
          bytes === null ? new Uint8Array() : new Uint8Array(bytes),
        ),
        options,
        uploaded: writtenAt,
...
describe("private learner dub API", () => {
  it("isolates sibling consent, status, storage keys, and reset fences", async () => {
    ...
    try {
      ...
      insertLearner.run("learner-a", "user-1", "Mia", "Mary", "mia");
      insertLearner.run("learner-b", "user-1", "Leo", "Rose", "leo");
  ...
  it("lets explicit recording consent clean up an incomplete deleting marker", async () => {
    const state = createTestD1Database();
    try {
      const timestamp = Date.parse("2026-08-25T08:00:00.000Z");
      state.sqlite
  ...
  it("reports missing recording access without granting it", async () => {
    const bucket = createBucket();
  ...
  it("grants only the exact bounded guardian consent object", async () => {
    ...
    for (const body of [
      ...
      JSON.stringify({
        accepted: true,
        consentVersion: "guardian-voice-r2-v1",
      }),
      JSON.stringify({
        accepted: true,
        consentVersion: CURRENT_CONSENT_VERSION,
  ...
  it("does not write when the captured consent generation is stale before put", async () => {
    ...
    const consentRepository = {
      ...
      async requireCurrentGrant(_userId, expectedGeneration) {
        checks += 1;
        return checks === 1 && expectedGeneration === undefined
          ? granted
          : null;
      },
    };
  ...
  it("stores validated learner peak bars atomically with the audio slot", async () => {
    ...
    assert.equal(upload.status, 201);
    assert.deepEqual(
      (await upload.json()).peakBars,
      peakBytes.map((peak) => peak / 255),
    );
    assert.equal(
      bucket.calls.put.find(
        ({ options }) => options.customMetadata?.state === "audio",
  ...
  it("writes Old MacDonald uploads only into the Old MacDonald namespace", async () => {
    ...
    assert.equal(
      bucket.stored.has(oldSlotKey("old-macdonald-v1-line-1")),
      true,
    );
  ...
  it("paces recent slot overwrites and reset marker finalization for R2 hot keys", async () => {
    ...
    assert.equal(
      clock.waits.every((delay) => delay >= 1_000),
      true,
    );
    assertResetTombstones(bucket, "reset-1");
  });
  ...
  it("preserves a newer post-reset take when stale same-line upload A settles later", async () => {
    ...
    assert.deepEqual(bucket.calls.delete, []);
    assert.deepEqual(
      storedAudio(bucket.stored.get(slotKey("line-1"))),
      repeatedBytes,
    );
    assert.deepEqual(
  ...
  it("marks malformed current envelopes unsaved and refuses their audio", async () => {
    ...
    const cases = [
      ...
      [
        "line-5",
        envelope.bytes.subarray(0, envelope.payloadOffset - 1),
        {
          generation,
          payloadOffset: String(envelope.payloadOffset),
          state: "audio",
      ...
      [
        "line-6",
        envelope.bytes.subarray(0, envelope.payloadOffset),
        {
          generation,
          payloadOffset: String(envelope.payloadOffset),
          state: "audio",
    ...
    const bucket = createBucket([
      ...
      ...cases.map(([lineId, bytes, customMetadata]) => [
        slotKey(lineId),
        {
          bytes,
          options: {
  ...
  it("keeps current markers and tombstones private and outside status rows", async () => {
    ...
    assert.equal(
      payload.lines.every(({ saved }) => !saved),
      true,
    );
    ...
    assert.equal(
      accountObjects.objects.some(({ key }) => key === MARKER_KEY),
      true,
    );
    assert.equal(
    ...
    assert.equal(
      accountObjects.objects.every(({ key }) => key.startsWith(ACCOUNT_PREFIX)),
      true,
    );
  ...
  it("does not let a pending retired reset delete its successor marker", async () => {
    ...
    assert.equal(
      bucket.calls.delete.some((key) => key === MARKER_KEY),
      false,
    );
    assertResetTombstones(bucket, "reset-2");
  });
    ...
    const cases = [
      {
        ...
        request: {
          body: new Uint8Array([0x1a, 0x45, 0xdf, 0xa3]),
          headers: { ...CONSENT_HEADERS, "Content-Type": "audio/wav" },
        },
      },
      {
        expected: 415,
      ...
      },
      {
        ...
        name: "MP4 mismatch",
        request: {
          body: new Uint8Array([0x4f, 0x67, 0x67, 0x53, 1, 2, 3, 4]),
          headers: { ...CONSENT_HEADERS, "Content-Type": "audio/mp4" },
        },
      },
      {
        expected: 415,
        ...
        request: {
          body: new Uint8Array([0x1a, 0x45, 0xdf, 0xa3]),
          headers: { ...CONSENT_HEADERS, "Content-Type": "audio/ogg" },
        },
      },
    ];
  ...
  it("rejects unknown dub and line IDs, empty clips, and oversized clips before R2 put", async () => {
    ...
    const cases = [
      [
        404,
        "/api/dubs/not-the-ducks/lines/line-1",
        new Uint8Array([0x1a, 0x45, 0xdf, 0xa3]),
      ],
      [
        404,
        `${DUB_PATH}/lines/line-99`,
        new Uint8Array([0x1a, 0x45, 0xdf, 0xa3]),
      ],
  ...
  it("rejects encoded canonical aliases before domain mutations", async () => {
    ...
    const beginRevocation =
      consentRepository.beginRevocation.bind(consentRepository);
