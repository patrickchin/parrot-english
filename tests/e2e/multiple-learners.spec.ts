...
async function openLearnerManagerFromDashboard(page: Page) {
...
async function activeLearnerId(page: Page) {
  return page.evaluate(
    () =>
      (
        window as Window & {
      ...
      ).__parrotE2eLearners?.snapshot().activeProfileId ?? null,
  );
}
...
test("chooses a sibling only while switching and keeps that session selection after refresh", async ({
  ...
  await expect(
    page.getByRole("button", { name: /Use .* learner mode/ }),
  ).toHaveCount(0);
...
test("active learner detail saves reach learner-mode consumers in the same SPA", async ({
  ...
  await chooseLearnerFromManager(page, "Mia Updated");
  await expect(page).toHaveURL("/");
  await expect(
    page.getByRole("button", {
  ...
  await expect(
    shelfPicker.getByRole("tab", { name: "Level 1 · Words & pictures" }),
  ...
  await expect(
    shelf
      .getByRole("tabpanel", { name: "Level 1 · Words & pictures" })
...
test("hides an unsaved Guardian profile edit during handoff and restores persisted data after returning", async ({
  ...
  try {
    ...
    await openLearnerManagerFromDashboard(draftPage);
...
test("keeps a saved URL-targeted learner update after another tab changes learner mode", async ({
  baseURL,
  browser,
}) => {
  if (!baseURL) throw new Error("Playwright baseURL is required.");
  ...
  try {
    ...
    await openLearnerManagerFromDashboard(profilePage);
    await expect(learnerCard(profilePage, "Mia Targeted")).toBeVisible();
    await expect(learnerCard(profilePage, "Noah")).not.toContainText(
      "Learner mode",
    );
    await expect.poll(() => activeLearnerId(profilePage)).toBe("learner-noah");
  } finally {
    await context.close();
  }
});

test("keeps conversations and dubbing isolated by selected learner", async ({
  page,
}) => {
  await page.goto(learnerScenarioUrl("/guardian/learners", "multiple"));
  await expect(
  ...
  const noahData = await page.evaluate(async ({ conversationId }) => {
    const [siblingConversation, dub] = await Promise.all([
  ...
  const storedRecordings = await page.evaluate(() => {
    const learners = (
      window as Window & {
        __parrotE2eLearners?: {
          snapshot(profileId?: string): {
            lessonRecording: { uploads: Array<{ outcome: string }> } | null;
          };
        };
      }
    ).__parrotE2eLearners;
    if (!learners) throw new Error("Learner controller is missing.");
    return {
      mia: learners.snapshot("learner-mia").lessonRecording?.uploads ?? [],
      noah: learners.snapshot("learner-noah").lessonRecording?.uploads ?? [],
    };
  });
...
test("automatically reveals a selection-required roster in Guardian mode", async ({
  ...
  await expect(page.getByRole("radio")).toHaveCount(0);
  const chooser = await openLearnerModeChooserFromManager(page);
...
test("reconciles a rejected learner selection without switching learner mode", async ({
  page,
}) => {
...
test("keeps the chooser modal and non-dismissible while a learner switch is pending", async ({
  page,
}) => {
  await page.goto(learnerScenarioUrl("/guardian/learners", "stale-selection"));
  const chooser = await openLearnerModeChooserFromManager(page);
...
for (const viewport of requiredViewports) {
  test(`keeps the CRUD roster, chooser, and Guardian menu contained at ${viewport.width}x${viewport.height}`, async ({
    page,
  }) => {
...
async function expectGuardianNameSurfacesContained(page: Page, name: string) {
  ...
  for (const surface of guardianNameSurfaces) {
    ...
    await trigger.click();
    const menu = page.getByRole("menu", { name: "Account menu" });
    await expect(menu.locator("..")).not.toContainText(`Managing ${name}`);
    await expect(
...
test("wraps an unbroken 120-character active learner name without horizontal overflow", async ({
  ...
  await trigger.click();
  const menu = page.getByRole("menu", { name: "Account menu" });
  await expect(menu.locator("..")).not.toContainText(`Managing ${longName}`);
  await expect(
    menu.locator("..").getByRole("group", { name: "Active profile" }),
  ).toHaveCount(0);
  await expect(menu.getByRole("menuitem")).toHaveText([
