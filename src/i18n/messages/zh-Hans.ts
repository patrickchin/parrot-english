import type { GuardianMessages } from "./en";

export const chineseGuardianMessages = {
  language: {
    controlLabel: "家长指导语言",
    englishOption: "English",
    chineseOption: "中文",
  },
  common: {
    cancel: "取消",
    retry: "重试",
    back: "返回",
    pageNavigation: "页面导航",
    save: "保存",
    saving: "正在保存…",
  },
  auth: {
    checkingSession: "正在检查登录状态…",
    sessionUnavailableTitle: "暂时无法登录",
    sessionUnavailableBody: "请检查网络连接，然后重试。",
    createAccountTitle: "创建账户",
    welcomeBackTitle: "欢迎回来",
    signUpHelp: "请使用家长姓名创建此账户。接下来可以添加孩子资料。",
    modeLabel: "选择登录或注册",
    signIn: "登录",
    signUp: "注册",
    accountName: "账户姓名",
    email: "电子邮箱",
    password: "密码",
    passwordHint: "至少 8 个字符",
    securityCheck: "安全验证",
    securityComplete: "安全验证已完成。现在可以以访客身份继续。",
    securityChecking: "正在进行安全验证。请稍候，完成后会自动启用访客访问。",
    securityUnavailable: "访客访问和注册暂时不可用。",
    securityLoadFailed: "安全验证无法加载。请刷新后重试。",
    securityHelp: "只有访客访问和新账户注册需要安全验证。",
    creatingAccount: "正在创建账户…",
    signingIn: "正在登录…",
    createAccount: "创建账户",
    signInAndStart: "登录并开始",
    or: "或",
    continuingAsGuest: "正在以访客身份继续…",
    continueAsGuest: "以访客身份继续",
    errors: {
      "name-required": "请输入姓名。",
      "invalid-email": "请输入有效的电子邮箱地址。",
      "password-too-short": "密码必须至少包含 8 个字符。",
      "email-registered": "此电子邮箱已注册。请改为登录。",
      "invalid-credentials": "电子邮箱或密码不正确。",
      "security-check-required": "请先完成安全验证，然后重试。",
      "security-check-rejected": "安全验证已过期或被拒绝。请重试。",
      "sign-in-failed": "无法登录。请重试。",
      "sign-out-failed": "退出登录未完成。",
    },
  },
  guardianAccess: {
    errors: {
      "check-failed": "无法检查家长访问权限。请重试。",
      "lock-failed": "无法锁定家长模式。交还设备前请重试。",
      "access-changed": "家长访问权限已更改。请重试。",
    },
  },
  guardianDashboard: {
    title: "家长中心",
    switchToLearner: "切换到学习模式",
    learnerProfilesTitle: "孩子资料",
    learnerProfilesDescription: "添加、编辑或删除孩子资料。",
    manageLearners: "管理孩子",
    accountPrivacyTitle: "账户与隐私",
    accountPrivacyDescription: "管理已保存的录音、隐私信息和账户删除。",
    openAccountPrivacy: "打开账户与隐私",
  },
  dubbingSettings: {
    backToDashboard: "返回家长中心",
    title: "配音管理",
    loading: "正在加载配音设置…",
    availableTitle: "可以使用配音",
    savedCount: (saved: number, total: number) =>
      `已保存 ${saved}/${total} 个片段；`,
    savedCountAfterName: " 可以在所有童谣中录制和替换台词。",
    privacyBeforeName: "已保存的配音片段仅属于 ",
    privacyAfterName: " 的私密档案。新录音会替换同一童谣台词的旧片段。",
    deleteAllGuidance: "此家长页面只会删除全部已保存片段，不提供逐句删除。",
    deleteBeforeName: "删除 ",
    deleteAfterName: " 已保存的童谣配音片段",
    deleting: "正在删除配音片段…",
    emptyTitle: "没有已保存的配音片段",
    emptyBeforeName: "",
    emptyAfterName: " 还没有已保存的童谣配音片段。",
    enableBeforeName: "允许 ",
    enableAfterName: " 录制并保存童谣配音片段",
    enabling: "正在开启配音…",
    enabledBeforeName: "现在可以为 ",
    enabledAfterName: " 使用配音了。",
    cleanupTitle: "需要完成配音片段删除",
    cleanupBeforeName: "在所有已保存片段删除完毕前，",
    cleanupAfterName: " 无法在任何童谣中使用配音。",
    finishCleanup: "继续删除童谣配音片段",
    removedBeforeName: "",
    removedAfterName: " 的童谣配音片段已删除。",
    errors: {
      "load-failed": "无法加载配音设置。请重试。",
      "change-failed": "无法更改配音设置。请重试。",
    },
  },
  accountPrivacy: {
    backToDashboard: "返回家长中心",
    title: "账户与隐私",
    savedVoiceClipsTitle: "已保存的配音片段",
    savedVoiceClipsDescription: "查看或删除已保存的童谣录音。",
    manageSavedVoiceClips: "管理已保存片段",
    technicalLabel: "技术构建详情",
    technicalTitle: "技术构建详情",
    technicalSubtitle: "用于故障排查的版本和 AI 服务",
    technicalBody:
      "当前服务包括用于托管的 Cloudflare、用于实时语音传输的 LiveKit、用于实时语音的 OpenAI，以及用于口语检查和资料摘要的 Groq。部分已保存的课程和资料音频是在部署前由 ElevenLabs 生成的。",
    webApp: "网页应用",
    worker: "Cloudflare Worker",
    agent: "对话代理",
    gitCommit: "Git 提交",
    deployment: "部署",
    uploaded: "上传时间",
    lastReported: "最近报告",
    realtimeModel: "实时语音模型",
    transcriptionModel: "输入转写模型",
    loadingTechnical: "正在加载技术详情…",
    technicalFailed: "无法加载技术详情。请稍后重试。",
    agentMissing: "对话代理尚未报告。开始对话时，它会报告自己的构建信息。",
    matchesWeb: "与网页提交一致",
    differsFromWeb: "与网页应用的提交不同",
    workerDeployment: (deploymentId: string) => `Worker 部署 ${deploymentId}`,
    missingValue: "暂无",
    dangerTitle: "危险操作区",
    dangerBody: "永久删除此账户及其已保存的孩子数据。",
    deleteAccount: "删除账户",
    deleteDialog: {
      cannotUndo: "此操作无法撤销",
      title: "删除账户",
      description:
        "这将从 Parrot 中删除你的账户、所有孩子资料、已保存的对话文字、所有童谣的私密配音片段和课程语音录音。系统会保留一个小型删除标记，防止已删除的私密媒体再次出现。",
      password: "密码",
      cancel: "取消",
      deleting: "正在删除账户…",
      confirm: "立即删除账户",
      errors: {
        "account-delete-failed": "无法删除账户。账户和私密媒体已保留，请重试。",
      },
    },
  },
  modeBoundary: {
    checkingTitle: "正在检查家长访问权限…",
    checkingDescription: "正在确认哪个档案可以使用此页面。",
    loadingTitle: "正在加载…",
    loadingDescription: "正在准备家长页面。",
    backToDashboard: "返回家长中心",
    openFailedTitle: "家长工具未能打开",
  },
  learnerSwitch: {
    title: "谁在学习？",
    description: "请选择谁来使用学习模式。",
    loading: "正在加载孩子资料…",
    loadFailed: "无法加载孩子资料。请重试。",
    profilesLabel: "孩子资料",
    empty: "切换到学习模式前，请先添加孩子。",
    manageLearners: "管理孩子",
    startAs: (name: string) => `以 ⁨${name}⁩ 身份开始学习模式`,
    starting: (name: string) => `正在以 ⁨${name}⁩ 身份开始…`,
    startingStatus: (name: string) => `正在以 ⁨${name}⁩ 身份启动学习模式…`,
    errors: {
      "select-failed": "无法选择这位孩子。请重试。",
      "lock-failed": "无法锁定家长模式。交还设备前请重试。",
    },
  },
  learnerTarget: {
    loading: "正在加载孩子设置…",
    loadFailed: "无法加载孩子资料。请重试。",
    noLearners: "还没有孩子资料。",
    addLearner: "添加孩子",
    invalidTarget: "页面链接中的孩子资料不存在。",
    manageLearners: "管理孩子",
    chooserLabel: "选择要设置的孩子",
    learnerMode: "学习模式",
    editingFor: (name: string) => `正在编辑 ⁨${name}⁩ 的设置`,
  },
  learners: {
    identityCheck: {
      checkingTitle: "正在检查当前孩子",
      checkingDescription: "请稍候，再进行更改。",
      failedTitle: "无法确认当前孩子",
      failedDescription: "请重试，确认更改会保存到正确的孩子资料后再继续。",
    },
    roster: {
      backToDashboard: "返回家长中心",
      backToDashboardAria: "返回家长中心",
      title: "管理孩子",
      loading: "正在加载孩子资料…",
      age: (age: number) => `${age} 岁`,
      ageMissing: "年龄未填写",
      setupStatuses: {
        completed: "设置已完成",
        in_progress: "设置进行中",
        not_started: "尚未开始设置",
      },
      editProfile: "编辑资料",
      editProfileAria: (name: string) => `编辑 ${name} 的资料`,
      delete: "删除",
      deleteAria: (name: string) => `删除 ${name}`,
      deleting: "正在删除 ",
      finishDeleting: "继续删除 ",
      finishDeletingAria: (name: string) => `继续删除 ${name}`,
      lastLearnerBefore: "请先添加另一位孩子，再删除 ",
      lastLearnerAfter: "。",
      addTitle: "添加孩子",
      addDescription: "请填写孩子喜欢使用的称呼，接下来可以补充其他资料。",
      preferredName: "常用名",
      adding: "正在添加孩子…",
      add: "添加孩子",
      deletedStatusBefore: "",
      deletedStatusAfter: " 已删除。",
      errors: {
        "load-failed": "无法加载孩子资料。请重试。",
        "add-failed": "无法添加孩子。请重试。",
        "last-learner": "请先添加另一位孩子，再删除这位孩子。",
        "learner-busy": "请先结束这位孩子当前的对话，然后重试。",
        "cleanup-pending": "孩子资料清理仍在进行中。请重试。",
        "deletion-uncertain":
          "无法确认这位孩子是否已删除。请刷新孩子资料后重试。",
        "delete-failed": "未能删除这位孩子。请重试。",
      },
    },
    deleteDialog: {
      cannotUndo: "此操作无法撤销",
      titleBefore: "删除 ",
      titleAfter: "？",
      descriptionBefore: "这将删除 ",
      descriptionAfter:
        " 的孩子资料和私密学习数据。家长账户和其他孩子资料不受影响。",
      alertBefore: "无法删除 ",
      alertAfter: "。",
      cancel: "取消",
      deleteBefore: "删除 ",
      deletingBefore: "正在删除 ",
    },
    details: {
      loadingTitle: "正在加载孩子资料…",
      loadingDescription: "正在准备这位孩子已保存的资料。",
      errorTitle: "孩子资料暂时无法使用",
      loadFailed: "无法加载孩子资料。请重试。",
      backToRoster: "返回管理孩子",
    },
    profile: {
      back: "返回",
      managing: (name: string) => `正在管理 ${name}`,
      managingBefore: "正在管理 ",
      managingAfter: "",
      title: "孩子资料",
      loading: "正在加载孩子资料…",
      loadErrorTitle: "孩子资料暂时无法使用",
      description: "这些资料用于个性化对话和课程。",
      savedAnswersHelper: "我们会保存你的回答，家长可以修改你的姓名和年龄。",
      name: "姓名",
      age: "年龄",
      aboutBefore: "关于 ",
      aboutAfter: "",
      aboutFallback: "这位孩子",
      descriptionPlaceholder: "添加简短介绍",
      recordingTitle: "课程语音录音",
      recordingDescription:
        "开启后，跟读环节可以为这位孩子保存语音片段。每个跟读环节仅保留最新片段，你可以随时在这里删除全部片段。",
      recordingEnabled: "课程语音录音已开启。",
      recordingDisabled: "课程语音录音已关闭。",
      recordingCleanupPending: "已保存的课程录音仍在删除中。",
      enableRecordings: "允许课程语音录音",
      deleteRecordings: "删除已保存的课程录音",
      finishDeletingRecordings: "继续删除课程录音",
      deleteRecordingsConfirm: "删除所有已保存的课程语音录音？此操作无法撤销。",
      saveChanges: "保存更改",
      saving: "正在保存…",
      cancel: "取消",
      pageErrors: {
        "load-failed": "无法加载孩子资料。请重试。",
        "save-failed": "无法保存孩子资料。请重试。",
        "recording-choice-failed": "无法保存课程录音设置。请重试。",
      },
      fieldErrors: {
        "answer-required": "请输入答案。",
        "question-unavailable": "这个问题已不可用。",
        "description-required": "请输入介绍。",
        "too-long": "请缩短回答后重试。",
        "private-details": "不要填写学校、住址、电话、电子邮箱或密码。",
        "preferred-name": "请只填写名字或常用昵称。",
        "age-whole-number": "请输入整数年龄。",
        "check-answer": "请检查答案后重试。",
      },
    },
    setup: {
      loading: "正在加载设置问题…",
      loadErrorTitle: "设置问题暂时无法使用",
      loadErrorDescription: "无法加载设置问题。请重试。",
      finishing: "正在完成孩子资料…",
      questionCount: (count: number, resuming: boolean) =>
        resuming ? `回答剩余 ${count} 个问题` : `回答 ${count} 个问题`,
      description: "我们会保存你的回答，家长可以修改你的姓名和年龄。",
      start: "开始回答",
      continue: "继续回答",
      skip: "暂时跳过",
      peppaAlt: "Peppa 挥手问好",
    },
    question: {
      progress: (current: number, total: number) => `问题 ${current}/${total}`,
      replay: "重播问题",
      peppaAlt: "你的英语主持人 Peppa",
      answer: "你的回答",
      speak: "说出你的回答",
      back: "返回",
      skipQuestion: "跳过这个问题",
      skip: "暂时跳过",
      save: "保存",
      next: "下一步",
      operationFailed: "操作未完成。请重试。",
      operationErrors: {
        "sound-start-failed": "无法播放声音。你可以继续，或点击扬声器按钮。",
        "sound-replay-failed": "无法播放声音。请再次点击扬声器按钮。",
        "voice-failed": "语音回答未完成。你仍然可以输入回答。",
        "try-again": "请重试。",
        "skip-failed": "暂时跳过未完成。",
        "question-skip-failed": "跳过这个问题未完成。",
      },
      statuses: {
        opening: "正在打开麦克风…",
        recording: "正在聆听…",
        transcribing: "正在整理文字…",
        saving: "正在处理…",
        ready: "已准备好。",
      },
    },
    acknowledgment: {
      next: "下一步",
      peppaAlt: "Peppa 微笑",
    },
  },
  account: {
    label: "账户",
    profileLabel: (name: string, mode: string) => `${name}的档案，${mode}模式`,
    signingOutProfileLabel: (profile: string) => `正在退出登录… ${profile}`,
    guardian: "家长",
    guardianModeStatus: "家长模式",
    learner: "学习者",
    learnerModeStatus: "学习模式",
    activeProfile: "当前档案",
    menuLabel: "账户菜单",
    switchLearner: "切换孩子",
    grownUpAccess: "家长入口",
    switchingModes: "正在切换模式…",
    guardianDashboard: "家长中心",
    signOut: "退出登录",
    signingOut: "正在退出登录…",
    signOutAgain: "再次退出登录",
  },
  learnerBoundary: {
    guardianAccessErrorHelper: "请让家长重试。",
    switchToLearnerHelper: "请家长切换到学习模式后继续。",
    savedAnswersHelper: "我们会保存你的回答，家长可以修改你的姓名和年龄。",
    recordingPermissionHelper: "请让家长开启麦克风权限，然后重试。",
  },
} as const satisfies GuardianMessages;
